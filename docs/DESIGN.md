...
| 事例の視覚素材は before/after 業務フロー図解 + 各デモシステムのスクリーンショット画像                  | 図解で変化を要約しつつ、公開デモ画面の画像で実在感を補強する。デモは公開情報のため匿名性・許諾不要方針と矛盾しない | [DETAIL: データ設計](./DESIGN_DETAIL_APP.md#データ設計)                                               |
...
- 実名事例の掲載・デモサイトへの外部リンク掲載 — 許諾が取れないため (公開デモ画面のスクリーンショット画像自体は掲載する。「決定一覧」参照)
...
- `CaseStudy` (新設): 匿名事例 1 件 (業種・規模感・支援範囲・課題・支援内容・変化・削減数値・図解ステップ・デモ画面画像)
